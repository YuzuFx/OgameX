...
| Poste de garde spatial (`space_dock`) | **[TBD]** | **Précision utile pour trancher** : ce n'est pas un "quai spatial" au sens naval — sa vraie fonction OGameX est de **réparer les vaisseaux endommagés** et de déterminer le ratio épaves/débris récupérables après une bataille (plus son niveau est haut, plus on récupère de coques réparables plutôt que du simple débris). Fonction utile et différenciante (pas un pur décor) — à retrancher si vraiment pas d'intérêt, ou à renommer en connaissance de cause (ex. lié à la Forge/aux artisans qui réparent) |
| Capteur/Phalanx (`sensor_phalanx`) | **[TBD]** | **Précision utile pour trancher** : détection à distance des mouvements de flottes ennemies dans les Éclats voisins (outil de renseignement défensif, complémentaire à l'espionnage classique) — pas un gadget mineur, c'est un vrai outil stratégique dans Ogame. À retrancher ou renommer (ex. lié à un Observatoire, une Tour de guet arcanique) en connaissance de cause |
...
| ~~Technologie hyperspatiale (`hyperspace_technology`)~~ | ~~**Supprimée ?**~~ | **Point à trancher** : pas de rôle "vitesse" à jouer (déjà couvert par le triptyque pied/cheval/siège ci-dessus), mais dans OGameX elle sert aussi de **prérequis de déblocage** pour plusieurs unités haut de tier (Croiseur de bataille, Bombardier, Destructeur, Étoile de la mort) et pour la technologie du graviton. Si on la supprime purement, il faut soit retirer ce prérequis pour ces unités, soit le reporter sur une des 3 techs de vitesse ci-dessus. Ton avis ? |
...
*Nourriture : la mécanique Énergie/centrale électrique est retirée (pas de dimension steampunk voulue, cf. 4.2). La Nourriture devient une **vraie 4ᵉ ressource produite et stockée** comme les 3 autres. **Piste ouverte le 2026-09-19, "à creuser" — pas encore décidée** : ajouter une consommation (upkeep) de Nourriture proportionnelle à la taille de l'armée, pour plafonner naturellement les armées surdimensionnées. ⚠️ **Attention, ça contredit le principe "pas d'upkeep, coûts ponctuels uniquement" acté plus haut en 4.1** ("la Nourriture suit les êtres vivants, jamais la pierre" — coût au recrutement, pas de consommation continue). Si cette piste est retenue, il faudra explicitement révoquer/reformuler le principe de 4.1, pas le laisser cohabiter tel quel avec une vraie mécanique d'upkeep.*
...
| Bâtiments et progression (5.2) | Oui, catalogue complet (18 objets Building+Station) | **Faible** — lexique quasi bouclé le 2026-09-19 (2 suppressions, 5 renommés, 3 encore en attente : usine de nanites, poste de garde spatial, phalanx) | `app/GameObjects/BuildingObjects.php`, `StationObjects.php` |
